	...
	private void updateAssignments(LocalDate date) {
		...
		while (today != null && today.hasNext()) {
			...
			assignmentBox.setStyle("-fx-background-color: rgba(" + classColor[0] + ", " + classColor[1] + ", "
					+ classColor[2] + ", 0.15); -fx-border-color: rgb(" + classColor[0] + ", " + classColor[1] + ", "
					+ classColor[2] + ");");
		...
		while (!datePlusOne.equals(endDate)) {
			...
			while (day != null && day.hasNext()) {
				...
				Text time = new Text(
						"Due on " + assignment.getDueDate().format(DateTimeFormatter.ofPattern("EEEE, MMMM d")) + " at " + assignment.getDueTime());
				time.setId("time");
